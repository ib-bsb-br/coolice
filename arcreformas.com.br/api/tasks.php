...
function handle_tasks_request(?string $board_slug): void {
    ...
    switch ($method) {
        case 'GET':
            $board_state = get_board_state($pdo, $board_slug);
            // ETag/Last-Modified logic
            $raw_content = json_encode($board_state);
            $etag = '"' . md5($raw_content) . '"';
            $last_mod_ts = $board_state['updated'] ?? time();
            $last_mod_header = gmdate('D, d M Y H:i:s', $last_mod_ts) . ' GMT';

            header('ETag: ' . $etag);
            header('Last-Modified: ' . $last_mod_header);

            if (($_SERVER['HTTP_IF_NONE_MATCH'] ?? '') === $etag || ($_SERVER['HTTP_IF_MODIFIED_SINCE'] ?? '') === $last_mod_header) {
                http_response_code(304);
                return;
            }
            emit_json($board_state);
            break;
        case 'POST':
...
function get_board_state(PDO $pdo, string $board_slug): array {
    ...
    $stmt = $pdo->prepare("SELECT id, text, is_done AS done, is_published, created_at AS ts FROM tasks WHERE board_slug = ? ORDER BY sort_order ASC, created_at ASC");
    $stmt->execute([$board_slug]);
    $tasks = $stmt->fetchAll();
...
function handle_task_operations(PDO $pdo, string $board_slug): void {
    ...
    try {
        ...
        Log::event('error', 'task_operation_failed', [
            'board' => $board_slug, 'operation' => $op, 'error' => $e->getMessage(),
        ]);
        emit_json(['error' => 'Database operation failed.'], 500);
        return;
    }
