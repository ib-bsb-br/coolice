...
{
    global $eventsFile;
    $ev = ['id' => bin2hex(random_bytes(8)),'ts' => time(),'type' => $type,'payload' => $payload];
    if ($fh = fopen($eventsFile, 'a')) {
        fwrite($fh, json_encode($ev)."\n");
        fclose($fh);
...
if ($slug !== '' && $op === '') {
    $st = $pdo->prepare('SELECT url FROM links WHERE slug=?');
    $st->execute([$slug]);
    if ($row = $st->fetch()) {
        $pdo->prepare('UPDATE links SET views=views+1 WHERE slug=?')->execute([$slug]);
        header('Location: ' . $row['url'], true, 302);
        exit;
    }
    http_response_code(404);
    echo '<!doctype html><meta charset="utf-8"><title>Not found</title><p>Short link not found. <a href="'.esc($domain).'">Create one</a>.</p>';
    exit;
}
...
if ($op === 'new-item' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $type = $in['type'] ?? 'file';
    if ($url === '') {
        emit_json(['error' => 'Missing url'], 400);
    }
...
if ($op === 'publish_md' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($token === 'your_github_personal_access_token_here') {
        ...
        if (is_file($ghTokenFile)) {
            $token = trim((string)file_get_contents($ghTokenFile));
        }
    }
    if ($token === '' || $token === 'your_github_personal_access_token_here') {
        emit_json(['status' => 'no_token','message' => 'GitHub token is not configured.'], 500);
    }
...
if ($op === 'task_add') {
    ...
    header('Location: ' . $memor_url, true, 303);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($op)) {
    ...
    $short = $domain . '/?s=' . $slug;
    echo '<!doctype html><meta charset="utf-8"><title>Shortened</title><p>Short link: <a href="'.esc($short).'">'.esc($short).'</a></p><p><a href="'.esc($domain).'">Create another</a></p>';
    exit;
}
