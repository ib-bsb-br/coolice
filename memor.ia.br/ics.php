...
function dt_ics(int $ts): string { return gmdate('Ymd\\THis\\Z', $ts); }
...
foreach ($tasks as $t) {
  ...
  $done = !empty($t['done']);
  $due = null;
  if (preg_match('/@(\\d{4}-\\d{2}-\\d{2})(?:\\s+|$)/', $summary, $m)) { $due = strtotime($m[1] . ' 09:00:00 UTC'); }
  ...
  if ($due) echo "DUE:" . dt_ics($due) . "\r\n";
  echo "STATUS:" . ($done ? "COMPLETED" : "NEEDS-ACTION") . "\r\n";
  if ($done) echo "PERCENT-COMPLETE:100\r\n";
  echo "END:VTODO\r\n";
}
echo "END:VCALENDAR\r\n";
